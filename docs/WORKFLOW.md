...
| list_style | string | 否 | 列表样式，默认 `multilevel` |

**list_style 取值**（用户自然语言可映射）：
...
**示例**：
```json
{ "type": "apply_list", "target_scope": "heading", "list_style": "decimal_hierarchical" }
{ "type": "apply_list", "target_scope": "heading", "list_style": "chinese_mixed" }
```
...
Z Mode 默认在修改 Word 前展示工作流供用户确认，用户可：

- **确认执行**：输入 `y` / `确认` / `执行` 后执行
- **补充修改**：直接输入修改意见（如「把第3步改成只设置正文边距」），系统会重新解析工作流并再次展示

### 确认流程

```
用户指令 → 解析工作流 → 展示预览 → 用户输入
                                    ├─ y/确认 → 执行
                                    └─ 补充内容 → 重新解析 → 展示预览 → ...
```
...
命令行加 `--no-confirm` 可跳过确认直接执行（用于脚本）：

```bash
python -m src.main doc.docx "1. 边距 2. 页码" -m z --no-confirm
```
...
# Z Mode：多步骤工作流（默认需确认，非交互环境会直接执行）
process_document(
...
# 交互确认（stdin）
process_document("report.docx", "添加页码；3个空行改1个", mode="z", z_interactive=True)

# 自定义确认回调（用于 UI 集成）
def my_confirm(workflow, prompt):
    print(format_workflow_for_display(workflow))
    r = input("确认? ")
    return (r.lower() == "y", None if r.lower() == "y" else r)
process_document("report.docx", "添加页码", mode="z", z_confirm_callback=my_confirm)
```
...
# 跳过确认（脚本用）
python -m src.main sample.docx "设置边距2.5cm；添加页码" -m z --no-confirm -o output.docx
```

### 自动模式判断

`mode="auto"` 时，由 **LLM 直接判断** 用户指令应使用 Y Mode 还是 Z Mode，无需规则匹配，鲁棒性更高。
...
| 标题用 1.1.1 格式 | `apply_list` list_style: decimal_hierarchical |
| 一级用一、二级用（一）、三级用1 | `apply_list` list_style: chinese_mixed |
